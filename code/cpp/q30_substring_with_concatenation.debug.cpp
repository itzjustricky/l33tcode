...
private:
    ...
    void tryToFreeTracker(int startPoint, int wordIndex, vector<bool>& tracker) {
        ...
        cout << "called tryToFreeTracker with startPoint=" << startPoint << ", "
             << "wordIndex=" << wordIndex << endl;

        while (wordsUsed > 0) {
            vector<int> wordIndices = _wordStartHash[startPoint];

            if (inVector(wordIndex, wordIndices)) {
                cout << "tracker successfully freed wordIndex " << wordIndex << endl;
                tracker[wordIndex] = false;
                break;
            } else {
                for (int x: wordIndices) {
                    if (tracker[x]) {
                        cout << "freeing tracker for wordIndex " << x << endl;
    ...
    void leapSearch(int leapStart, vector<bool>& doneIndices, vector<int>& startIndices) {
        ...
        cout << "//////////////////////////////////////////////////" << endl;
        cout << "start leap search with leapStart " << leapStart << endl;
    ...
    vector<int> findSubstring(string s, vector<string>& words) {
        ...
        for (size_t i = 0; i <= (s.size() - _wordLength); ++i) {
            ...
            if (wordIndices.size() > 0) {
                cout << "adding word, start: " << i << " word: "
                     << s.substr(i, _wordLength) << endl;
                _wordStartHash[i] = wordIndices;
            }
        }
